\subsection{Decorrelation by Whiteninig}
...
% --- SLIDE 6.6: VISUAL INTUITION (GEOMETRY - FIXED) ---
\begin{frame}{Conceptual Comparison: Visual Geometry (2/2)}
    ...
    \begin{tikzpicture}[scale=1.2, transform shape]
        % --- STEP 1: ORIGINAL ---
        \node (origin) at (0,0) {\includegraphics[width=2cm]{original_correlated_data}};
        \node[below=0.1cm] at (origin.south) {\small \textbf{Original}};
        \node[below=0.5cm, gray] at (origin.south) {\scriptsize (Correlated)};

        % --- ARROW 1: PCA (FIXED) ---
        % Gắn node chữ trực tiếp vào lệnh draw để không bị trôi
        \draw[->, thick, blue] (1.3,0) -- (3.2,0)
            node[midway, above] {\scriptsize $\vect{P}^T$}
            node[midway, below] {\scriptsize \textbf{Rotate}};

        % --- STEP 2: PCA SPACE ---
        \onslide<2->{
            \node (pca) at (4.5,0) {\includegraphics[width=2cm]{PCA_data}};
            \node[below=0.1cm] at (pca.south) {\small \textbf{PCA Space}};
            \node[below=0.5cm, gray] at (pca.south) {\scriptsize (Elliptical)};
        }

        % --- ARROW 2: WHITENING (FIXED) ---
        \onslide<3->{
            \draw[->, thick, red] (5.8,0) -- (7.7,0)
                node[midway, above] {\scriptsize $\vect{\Lambda}^{-1/2}$}
                node[midway, below] {\scriptsize \textbf{Scale}};

            % --- STEP 3: WHITENED SPACE ---
            \node (white) at (9.0,0) {\includegraphics[width=2cm]{whited_data}};
            \node[below=0.1cm] at (white.south) {\small \textbf{Whitened}};
            \node[below=0.5cm, gray] at (white.south) {\scriptsize (Spherical)};
        }
    \end{tikzpicture}
...
% --- SLIDE 5.1: FULL FORMULA COMPARISON (COMPACT & OPTIMIZED) ---
\begin{frame}{The "Lost Sibling" Revealed (2/2)}
    \small
    % Mẹo Senior: Co khoảng cách quanh công thức toán lại
    ...
    % Phần chuyển tiếp: Thu gọn khoảng cách tối đa
    \vspace{-0.2em}
    \begin{center}
    ...
    % 2. THE WHITENING FORMULA
    \onslide<1->{
        \textbf{2. The Whitening Form (Eq. 10.44):}
        \begin{equation*}
            \hat{f}(\vect{x}) = \frac{\alert<2>{|\hat{\mathbf{\Sigma}}|^{-1/2}}}{n h^p} \sum_{i=1}^{n} K \left( \alert<3>{\frac{\hat{\mathbf{\Sigma}}^{-1} (\vect{x} - \vect{X}_i)}{h}} \right)
        \end{equation*}
    }
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.28\textwidth}
            ...
            \begin{itemize}
                ...
                \raggedright
                \item \textbf{Mean:} 1G/A, 9km.
                \item \textbf{A:} 1G/A, \textbf{11km} (Easy).
                \item \textbf{Messi:} \textbf{3G/A}, 9km (Hard).
            \end{itemize}
        \end{column}
...
\begin{frame}{Intuition 2: The "All-Rounder" Anomaly (Correlation)}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.28\textwidth}
            ...
            \includegraphics[width=1.0\linewidth, height=3.0cm, keepaspectratio]{orange_singer}
